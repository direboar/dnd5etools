...
test('testParseAttack', () => {
    ...
    expect(ret[0].damageRole).toBe('1d6+1  slashing damage.  ')
    expect(ret[1].name).toBe('Light Crossbow.')
    expect(ret[1].attackRole).toBe('+3')
    expect(ret[1].damageRole).toBe('1d8+1  piercing damage. ')
    ...
    expect(ret[0].damageRole).toBe('3d8+8  slashing damage plus 13 (3d8) lightning damage. If the balor scores a critical hit, it rolls damage dice three times, instead of twice.  ')
    expect(ret[1].name).toBe('Whip.')
    expect(ret[1].attackRole).toBe('+14')
    expect(ret[1].damageRole).toBe('2d6+8  slashing damage plus 10 (3d6) fire damage, and the target must succeed on a DC 20 Strength saving throw or be pulled up to 25 feet toward the balor.  ')
    ...
    expect(ret[0].damageRole).toBe('1d8+4  bludgeoning damage. Instead of dealing damage, the vampire can grapple the target (escape DC 18).  ')
    expect(ret[1].name).toBe('Bite. (Bat or Vampire Form Only).')
    expect(ret[1].attackRole).toBe('+9')
    expect(ret[1].damageRole).toBe("1d6+4  piercing damage plus 10 (3d6) necrotic damage. The target's hit point maximum is reduced by an amount equal to the necrotic damage taken, and the vampire regains hit points equal to that amount. The reduction lasts until the target finishes a long rest. The target dies if this effect reduces its hit point maximum to 0. A humanoid slain in this way and then buried in the ground rises the following night as a vampire spawn under the vampire's control.  ")
    ...
    expect(ret[0].damageRole).toBe('1d6  bludgeoning damage, 4 (1d8) bludgeoning damage if wielded with two hands, or 6 (1d8 + 2) bludgeoning damage with shillelagh. ')
